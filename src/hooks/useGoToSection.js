...
export function useGoToSection(){
  ...
  return (section) => {
    try {
      ...
      if (!atLanding || current !== section){ navigate(base,{replace:false}) }
      else { navigate(base+`&r=${Date.now()}`,{replace:true}) }
      let tries = 0
      const tryScroll = () => {
        ...
        } else if (tries < 12){ tries++; requestAnimationFrame(tryScroll) }
      }
      requestAnimationFrame(tryScroll)
    } catch (e) { console.error('useGoToSection error:', e) }
  }
}
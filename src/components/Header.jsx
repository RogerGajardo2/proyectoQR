...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 h-[var(--nav-h)] border-b border-line bg-white md:glass transition-transform duration-300 ${show ? 'translate-y-0' : '-translate-y-full'}`}>
      ...
      {/* Backdrop para mobile (debajo del drawer) */}
      <button
        aria-hidden
      ...
      {/* Menú móvil: fondo blanco sólido, altura completa, 50% del ancho desde la derecha */}
      <nav
        id="primary-nav"
        className={`md:hidden fixed top-[var(--nav-h)] bottom-0 right-0 w-1/2 min-w-[280px] bg-white border-l border-line shadow-2xl transition-transform duration-300 z-[60] isolate ${open ? 'translate-x-0' : 'translate-x-full'}`}
      >

...
export default function ProjectsCarousel(){
  ...
  useEffect(() => {
    const start = () => timer.current = setInterval(() => goTo(idx + 1), 6000)
  ...
  return (
    <section id="proyectos" className="py-16 bg-alt">
      <div className="container">
        ...
        <div className="relative overflow-hidden rounded-2xl shadow-soft bg-white" data-reveal>
          ...
          {/* Controles */}
          <button onClick={()=>goTo(idx-1)} aria-label="Anterior" className="absolute top-1/2 -translate-y-1/2 left-3 grid place-items-center w-11 h-11 bg-white/85 border border-line rounded-xl shadow-soft hover:bg-white">‹</button>
          <button onClick={()=>goTo(idx+1)} aria-label="Siguiente" className="absolute top-1/2 -translate-y-1/2 right-3 grid place-items-center w-11 h-11 bg-white/85 border border-line rounded-xl shadow-soft hover:bg-white">›</button>
